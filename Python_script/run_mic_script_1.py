...
def get_argument():
    ...
    Parser.add_argument("-s", "--serialPort", help="serialPort")
...
if __name__ == '__main__':
    func_list = [open_serial_port, read_product_mlb, read_product_fw, mic1_init,
                 mic1_test, mic2_init, mic2_test, close_serial_port, device_reset]
    args = get_argument()
    # 检查arg参数的长度
    if args.serialPort is None  or args.logPath is None :
        print("Please input all parameters.")
        exit()
    virtual_port = args.serialPort
    log = logDriver.LogDriver("{}.log".format(log_file))
    time.sleep(0.5)
