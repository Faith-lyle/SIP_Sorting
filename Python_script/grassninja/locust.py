...
#        print(resp)
    ...
    def sendReset(self, destination = "HostLocust", reset_mode = 0x01):
        ...
        resp = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=Cmd["B2P_CMD_RESET"], payload=payload, timeout=0.3, verb=True, get_resp=True, addDum = False)
        return resp
    ...
    def writeLagunaReg(self,register,data,addDum=False): 
        self.writeI2C(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, data=data, wtype="reg16", register=register, destination = 'DeviceLocust',addDum=addDum)
        return
    

    def readI2CRegs(self, bus, slave_addr, read_len, register=0x00, rtype="reg8", destination = "HostLocust", timeout=0.5):
        read_type = {
            "read": 0x1A,
            "reg8": 0x1C,
            "reg16": 0x1E
        ...
        resp = self.b2p_uart.SendRead(rcode=routing_code[destination], cmd=read_type_tosend, payload=complete_data_payload, timeout=timeout, verb=True)  
        if resp is False:
            if self.debug:
    ...
    def readLocustReg(self,register,target='host',n=1):
        ...
        if target == 'device': 
            destination = 'DeviceLocust'
        result = self.readI2CRegs(rtype = "reg8", destination = destination, bus = 0x00, slave_addr = 0x33, register = register, read_len = n)
        return result

    def readLagunaReg(self,register,n=1):
        resp = self.readI2CRegs(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, read_len=n, rtype="reg16", register=register, destination = 'DeviceLocust')
        return resp
    ...
    def sendLoopback(self, data, destination = "HostLocust", quiet = False):
        ...
        try:
            ...
            RuntimeError("GN host - locustb2p: Error - resp incorrect, b2p path may not work")
