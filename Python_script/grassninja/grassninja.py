...
class grassNinjaHost():
    def __init__(self, uart,i2cPortName,debug=False): 
        self.debug = debug
        ...
        self.locustB2P = locust(self.b2pUart,debug=debug) # Access to locust host, slave, laguna and durant
    ...
    def setTimeouts(self):
        ...
        devCFG = self.locustB2P.readLocustReg(0x55, target='device', n=2) # 4:0, 10ms to 320ms, mid_remote_timeout
        hostCFG5= hostCFG[0]
        devCFG5 = devCFG[0]
        hostCFG6= hostCFG[1]
        devCFG6 = devCFG[1]
        ...
        self.locustB2P.writeLocustReg(0x55, [devcfg5,devcfg6], target='device') # 4:0, 10ms to 320ms, mid_remote_timeout
...
#            "UVP": 0x5 #Do we need UVP
    ...
    def forceLagunaPowerState(self, state):
        ...
        if state_val[state] is None:
            print("GN host: power state invalid")
            return
        readback = self.locustB2P.readI2CRegs(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, read_len=0x1, register=REG_FROCE_DFU, rtype="reg16", destination = destination)
        if readback is None:
            print("GN host: Error, b2p not able to readback data, check funbus connection")
            return
        value_to_write = readback[0] & 0xF8 | state_val[state]
        self.locustB2P.writeI2C(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, data=value_to_write, wtype="reg16", register=REG_FROCE_DFU, destination = destination)
        return True
...
#            self.locustB2P.writeI2C(bus=0x01, slave_addr=LAGUNA_I2C_7BIT_ADDR, data=0x00, wtype="reg16", register=0x1868, destination = "DeviceLocust")
    ...
    def setCommsMode(self, mode="DPM"):
        ...
        print("GN host: Set confirm COMMS bit of device")
        # time.sleep(1)
        self.locustB2P.getState("DeviceLocust")
        self.locustB2P.writeI2C(bus=0x00, slave_addr=0x33, data=0x1, wtype="reg8", register=REG_CONFIRM_COMMS, destination = "DeviceLocust", timeout=0.2)
    ...
    def powerCycle(self):
        '''
        PowerCycle the grasssnjia
        '''
        self.ioexp.powerCycle()
