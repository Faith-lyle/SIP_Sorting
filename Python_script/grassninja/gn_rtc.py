...
if __name__ == '__main__': 
    ...
    gnTest.lagunaIsink(current=0x5,target='amuxout_ax') # disable sink current
    gnTest.lagunaAmuxSel(src='disabled',target='ax')

    if 'on' in state: 
        gnTest.lagunaAmuxSelRTC(pin='ay',on=True)
    else: 
        gnTest.lagunaAmuxSelRTC(pin='ay',on=False)
